...
- A first 15-minute `500` offered TPS steady-state attempt was run and rejected: the driver maintained `500.00` BUY confirmations/s, but the run used a development Redis configured with `allkeys-lru`, which evicted order detail keys and invalidated MatchEngine order-book state.
...
- A valid 10-15 minute steady-state throughput claim is still pending until the run is repeated on clean `noeviction` loadtest Redis.
