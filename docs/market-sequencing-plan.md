...
> EAP 目前已具備 wallet-first、outbox、DLQ、Redis Lua atomic matching 與 audit replay。接近真交易所還缺一個核心 primitive：per-market sequencer。我的規劃是讓每個 market 自己有一條 `marketSequence`，order book 以 price + sequence 維持 price-time priority。Audit 則從全域 hash chain 調整為 per-order lifecycle chain，市場順序由 `marketSequence` 證明，訂單狀態完整性由 per-order hash chain 證明。這讓 order service 可以多 pod 接單，同時維持市場公平性。
