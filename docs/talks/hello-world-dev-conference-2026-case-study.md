...
**講綱提交後的證據更新。** 2026-09-03 的可靠性版本新增三服務 durable inbox 與 Order 雙狀態，舊版 648 不能直接移植。新版嚴格量測 RabbitMQ 與 Order service-owned inbox 後，單一 seed 在 200 orders/s 通過，300／400 因 inbox 持續累積而拒絕，即使所有資料最後仍正確收斂。這不改變已提交案例的主旨，反而增加一個更直接的例子：版本與完成定義改變時，AI 或人都不能沿用舊 TPS；`queue=0` 也不等於服務內工作完成。
