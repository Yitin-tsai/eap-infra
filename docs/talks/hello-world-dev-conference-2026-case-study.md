...
EAP 不宣稱 `2000 completed TPS`。目前 release-pinned 公開下界是 2 個 workload seed 都通過的單機 `600 accepted orders/s` 等級 CDA 隨機混合 HTTP 持續證據；`624` 有 1 次有效 15 分鐘候選，但尚未跨長窗 seed 重複，`648` 只有 1 次探索性短窗通過。較舊版本另有單機短時間 `699.98 accepted orders/s`、`346.12 same-window trades/s`、`320.47 full-lifecycle trades/s` 的歷史診斷。`922.38/s` 是 CDA 依序上限診斷；`20405.04/s` 是已拒絕版本的隔離診斷。已確認訂單、混合流量、短時間窗、長時間、元件隔離、歷史版本、深度診斷與 TDA 必須分開。
