...
Redis is the hot matching state, not the long-term audit source of truth. PostgreSQL keeps command-side order facts and `TradeExecuted` facts. MatchEngine now persists an authoritative `READY` or `RECOVERING` control row with a monotonic fence epoch, generation UUID, Redis `run_id`, CAS version, and verification manifest. Every CDA mutation Lua script checks both the generation sentinel and the live Redis `run_id` before its first write. If the Redis generation is lost or cannot be trusted, admission workers, cancellation arbitration, and Redis maintenance fail closed while the Rabbit listener may continue durable inbox intake.

Reopening is an operator-controlled protocol: a request is bound to the current epoch, generation, and version; MatchEngine serializes activation, validates durable watermarks and the Redis structure/identity manifest, requires every Redis completed-admission bitmap bit to match an `APPLIED` PostgreSQL admission-inbox fact, and checks cancellation intents/markers against durable decisions. Activation holds an exclusive PostgreSQL advisory lock while cancellation durable intake uses the shared mode of the same lock, so cancellations remain concurrent but an activation cannot miss an in-flight `PENDING` fact between its final snapshot and `READY` CAS. A cancellation marker may not coexist with the same visible order. Control transitions compare version, epoch, generation, and Redis run ID so a pre-reset snapshot cannot demote a reinitialized generation. MatchEngine stages the sentinel, validates again, and finally marks PostgreSQL `READY` with CAS. These full checks run only on the recovery control path, not as a PostgreSQL lookup on every match. Automatic full-book rebuild is still not implemented, and matching is never claimed to continue through Redis state loss; durable rebuild input remains `EAP-MATCH-202`.

Ordinary runtime status checks only the control row, sentinel, and Redis `run_id`. Full-manifest inspection is a separate non-destructive diagnostic that is meaningful only after queues, inbox work, reservations, and cleanup have converged. Legal in-flight shapes—such as a reserved detail temporarily absent from its ZSET or a completed bitmap bit written before the durable inbox reaches `APPLIED`—must not let a GET status demote the generation. Only quiescent full verification during `RECOVERING` activation has promotion authority.
