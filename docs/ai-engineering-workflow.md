...
這個案例同時保留「採用」與「否決」：採用有完整證據支持的排程隔離，否決未通過最終一致性檢查的高 TPS 結果。公開的同機混合 HTTP 證據只保留短時間約 700 accepted orders/s 的下界等級，不延伸成目前版本的 15 分鐘持續容量宣稱。證據見 [2026-08-07 現行混合流量診斷](benchmarks/2026-08-07-canonical-mixed-http-diagnostic.md)與 [2026-08-11 release-pinned 700 重測](benchmarks/2026-08-11-release-pinned-700-and-recovery-ownership.md)。
