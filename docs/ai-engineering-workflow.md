...
個人開發時，同一個人很容易連續扮演需求提出者、架構師、實作者、測試者與審查者，最後只找支持原始想法的證據。AI 雖然能加快分析與實作，也可能同步放大這種確認偏誤：更快產生一個看似完整的方案，不代表前提、交易邊界或驗收方式正確。EAP 以多角色 AI 工作流刻意切斷這條思考捷徑：
...
這些角色不等於真實組織中的獨立人員或簽核者，也不能取代正式 code review、Code Owner 或變更核准。它們的作用是建立有順序、有權限限制、可以重複執行的個人自我審查程序，強迫方案在實作前後都面對結構化反對意見。

角色也不是只存在於本文件中的名稱。EAP 將角色責任、禁止事項、輸出格式與停止條件保存成版本控制的 [Codex skills](#實際工作流工件)；工程師可以檢查某次工作使用了哪一種角色契約，但是否接受輸出仍是人工決策。
...
這種安排的目的不是增加代理數量，也不是假裝獲得多人團隊的獨立審查，而是讓不同觀點在實作前後形成可追溯的反對意見。
...
後續 2026-09 可靠性改版再次證明相同原則：三服務加入 durable inbox 後，舊 648 數字不再代表目前寫入路徑；而且壓測發現 Rabbit queue 已歸零時，Order inbox 仍可累積數萬筆。工具補上 service-owned debt gate 後，原本顯示通過的 300 被人工審查推翻；Wallet trade inbox 加入後又先把舊 200 降級成 baseline。2026-09-04 再以三服務 inbox backlog／age／terminal debt、final outbox／cleanup 及 schema fail-closed gate 重跑，才恢復單一 seed 的 200 orders/s 診斷下界。這是「程式與驗證工具都要被 review，而且證據要隨版本重建」的最新案例。
