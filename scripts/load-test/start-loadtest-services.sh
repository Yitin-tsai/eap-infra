...
start_service() {
  ...
  if [[ "${repo}" == "eap-order" ]]; then
    service_env="EAP_ORDER_OUTBOX_BATCH_SIZE='${EAP_ORDER_OUTBOX_BATCH_SIZE:-500}' EAP_ORDER_OUTBOX_BATCH_CONFIRM_ENABLED='${EAP_ORDER_OUTBOX_BATCH_CONFIRM_ENABLED:-true}' EAP_ORDER_OUTBOX_ASYNC_RELAY_ENABLED='${EAP_ORDER_OUTBOX_ASYNC_RELAY_ENABLED:-false}' EAP_ORDER_OUTBOX_IN_FLIGHT_TIMEOUT_SECONDS='${EAP_ORDER_OUTBOX_IN_FLIGHT_TIMEOUT_SECONDS:-30}' EAP_ORDER_ASSET_RESERVATION_CONFIRMED_BATCH_SIZE='${EAP_ORDER_ASSET_RESERVATION_CONFIRMED_BATCH_SIZE:-50}' EAP_ORDER_ASSET_RESERVATION_CONFIRMED_RECEIVE_TIMEOUT_MS='${EAP_ORDER_ASSET_RESERVATION_CONFIRMED_RECEIVE_TIMEOUT_MS:-25}'"
  fi
