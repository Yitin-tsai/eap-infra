...
GRADLE_USER_HOME="${GRADLE_USER_HOME_DIR}" ./gradlew --no-daemon matchedE2eLoadTest --args="--phase ${PHASE} --events ${EVENTS} --publishers ${PUBLISHERS} --timeout-seconds ${TIMEOUT_SECONDS} --market-id ${MARKET_ID}" | tee "${REPORT_FILE}"
