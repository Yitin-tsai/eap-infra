...
ORDER_ADMISSION_WALLET_OUTBOX_BATCH_SIZE="${ORDER_ADMISSION_WALLET_OUTBOX_BATCH_SIZE:-500}"
...
Common options:
  ...
  --asset-confirmed-collector-count VALUE
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --asset-confirmed-collector-count)
      [[ $# -ge 2 ]] || { echo "[ERROR] --asset-confirmed-collector-count requires a value" >&2; exit 2; }
      ORDER_ADMISSION_ASSET_RESERVATION_CONFIRMED_CONCURRENCY="$2"
      shift 2
      ;;
...
if [[ "${START_SERVICES}" == "true" ]]; then
  EAP_MATCH_USER_OPEN_ORDER_INDEX_ENABLED="${ORDER_ADMISSION_MATCH_USER_OPEN_ORDER_INDEX_ENABLED}" \
    ...
    EAP_WALLET_OUTBOX_BATCH_SIZE="${ORDER_ADMISSION_WALLET_OUTBOX_BATCH_SIZE}" \
...
echo "[INFO] walletOutboxBatchSize=${ORDER_ADMISSION_WALLET_OUTBOX_BATCH_SIZE}, walletOutboxAsyncRelayEnabled=${ORDER_ADMISSION_WALLET_OUTBOX_ASYNC_RELAY_ENABLED}, walletOutboxAsyncMaxInFlightBatches=${ORDER_ADMISSION_WALLET_OUTBOX_ASYNC_MAX_IN_FLIGHT_BATCHES}"
