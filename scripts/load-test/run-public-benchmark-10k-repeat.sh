...
if [[ "${DIAGNOSTICS_LEVEL}" != "none" && "${DIAGNOSTICS_LEVEL}" != "light" ]]; then
  echo "[ERROR] public benchmark should use none/light diagnostics; got DIAGNOSTICS_LEVEL=${DIAGNOSTICS_LEVEL}." >&2
  exit 2
fi
